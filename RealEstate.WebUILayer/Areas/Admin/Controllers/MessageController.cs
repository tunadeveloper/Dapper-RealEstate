...
namespace RealEstate.WebUILayer.Areas.Admin.Controllers
{
    ...
    public class MessageController : Controller
    {
        ...
        public async Task<IActionResult> Index(int? page)
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7175/api/Messages");
        ...
        public async Task<IActionResult> CreateMessage(CreateMessageDTO createMessageDTO)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7175/api/Messages", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7175/api/Messages/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7175/api/Messages/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateMessage(UpdateMessageDTO updateMessageDTO)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7175/api/Messages", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
